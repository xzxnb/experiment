...
def edge_weight(mln: str):
    ...
    wfomcs_problem = MLN_to_WFOMC1(mln_problem, '@F')
...
if __name__ == '__main__':
    ...
    weight1 = [0 for i in range(2 * m)]
    weight2 = [0 for i in range(2 * m)]
    for i in range(2 * m):
        weight1[i] = newton(f_weight1 - v1[i], x1, 1, 0.001, 100)
        weight2[i] = newton(f_weight2 - v2[i], x2, 1, 0.001, 100)
    result = []
    w1 = create_vars("w1")
    w2 = create_vars("w2")
    ...
    for i in range(2*m):
        for j in range(2*m):
            result.append([weight1[i], weight2[j], v1[i], v2[j], res.subs({w1: weight1[i], w2: weight2[j]})])
    df = pd.DataFrame(result, columns=["weight1", "weight2", "edge1", "edge2", "TV"])
    excel_filename = "domain10.xlsx"
